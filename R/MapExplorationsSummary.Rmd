...
```{r}
library(ggmap)
map1 <- get_map(c(-118.2500, 34.0500), zoom = 10, maptype = "toner", source = "stamen")
map1 <- ggmap(map1)
map1
```
...
The shapefile data is provided by the Census, so it's reasonably easy to load the shapefiles and plot them over the base map. The data is available here: ftp://ftp2.census.gov/geo/tiger/TIGER2013/TRACT/
```{r}
# require(rgdal)
##This reads in the shapefile data and then just grabs the LA stuff.
#tracts <- readOGR("tl_2013_06_tract.shp", "tl_2013_06_tract")
...
require(ggplot2)
# read in data
LAtracts <- readOGR("../data/shapes/tl_2013_06_tract/.", layer="LAtracts")
...
```{r tractmap}
plainpolys <- map1 + geom_polygon(aes(x=long, y=lat, group=id), size=.2, color='green', data=LAtracts.df,fill=NA)
plainpolys
```
...
For a spatial aggregation level above the tract size, I found data about the boundaries of LA neighborhoods. It is available online: http://www.arcgis.com/home/item.html?id=7da35609c7604967bc2e99edf6b0100e
```{r}
...
### Zipcodes
Next we use zipcodes. That data came from here: http://egis3.lacounty.gov/dataportal/2011/01/24/zip-codes-with-parcel-specific-boundaries/
